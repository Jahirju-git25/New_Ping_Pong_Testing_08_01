...
static NETSIM_ID find_virtual_mac_interface(NETSIM_ID d, NETSIM_ID in, UINT ac)
{
	...
	return fn_NetSim_Stack_FindVirtualInterface(d, in, virtualInterfaceName[ac]);
}
...
int fn_NetSim_IEEE802_EDCA_Init(NETSIM_ID nDeviceId, NETSIM_ID nInterfaceId)
{
	...
	for(i=0;i<MAX_AC_CATEGORY;i++)
		pstruMacVar->dot11EdcaTable[i].qosList.dMaximumSize = DEVICE_ACCESSBUFFER(nDeviceId, nInterfaceId)->dMaxBufferSize;
...
NETSIM_ID fn_NetSim_IEEE802_11e_AddtoQueue(NETSIM_ID d, NETSIM_ID in, NetSim_PACKET* packet)
{
	...
	if (mac->mediumAccessProtocol == DCF)
		sprintf(vinName, "%s", "N/A");
	else
		sprintf(vinName, "%s", virtualInterfaceName[ac]);
	...
	{
		fnNetSim_buffer_log_write("%lf,%d,%s,%d,Interface_%d (%s),%lf,%lf,%s,%s", 
			pstruEventDetails->dEventTime, d, DEVICE_NAME(d), in, in, 
			DEVICE_INTERFACE(d, in)->szInterfaceName, mac->currEdcaTable->qosList.dCurrentSize, 
			mac->currEdcaTable->qosList.dMaximumSize / (1024 * 1024), vinName, "DROP");
		packet->nPacketStatus = PacketStatus_Buffer_Dropped;
	...
	fnNetSim_buffer_log_write("%lf,%d,%s,%d,Interface_%d (%s),%lf,%lf,%s,%s", 
		pstruEventDetails->dEventTime, d, DEVICE_NAME(d),in,in, 
		DEVICE_INTERFACE(d, in)->szInterfaceName, mac->currEdcaTable->qosList.dCurrentSize, 
		mac->currEdcaTable->qosList.dMaximumSize / (1024 * 1024), vinName,"ENQUEUE");
	return vin;
}

static NetSim_PACKET* get_packet_from_qoslist(QOS_LIST* list, NETSIM_ID recvId)
{
	...
	{
		NetSim_PACKET* packet = list->head;
		NetSim_PACKET* prev = NULL;
		while (packet && packet->nReceiverId != recvId)
		{
			prev = packet;
		...
		if (packet)
		{
			if (prev)
			{
				prev->pstruNextPacket = packet->pstruNextPacket;
				packet->pstruNextPacket = NULL;
				if (packet == list->tail) { list->tail = prev; }
			}
			else
			{
				list->head = packet->pstruNextPacket;
				packet->pstruNextPacket = NULL;
				if (list->head == NULL) list->tail = NULL;
			}
			p = packet;
														...
														UINT nPacketRequire, UINT* nPacketCount)
{
	...
	for (i = 0; i < nPacketRequire; i++)
	{
		...
		UINT ac = get_ac_from_qos(t->nQOS);
		NETSIM_ID vin = find_virtual_mac_interface(d, in, ac);
		...
		{
			fnNetSim_buffer_log_write("%lf,%d,%s,%d,Interface_%d (%s),%lf,%lf,%s,%s", 
				pstruEventDetails->dEventTime, d, DEVICE_NAME(d),
				in, in, DEVICE_INTERFACE(d, in)->szInterfaceName, mac->currEdcaTable->qosList.dCurrentSize, 
				mac->currEdcaTable->qosList.dMaximumSize / (1024 * 1024), 
				vinName, "LIFE TIME EXPIRED");
			t->nPacketStatus = PacketStatus_MAC_Lifetime_Expired;
			if (t->nReceiverId == 0) t->nReceiverId = pstruEventDetails->nDeviceId;
		...
		*nPacketCount += 1;
		fnNetSim_buffer_log_write("%lf,%d,%s,%d,Interface_%d (%s),%lf,%lf,%s,%s", 
			pstruEventDetails->dEventTime, d, DEVICE_NAME(d),in, in, 
			DEVICE_INTERFACE(d, in)->szInterfaceName, mac->currEdcaTable->qosList.dCurrentSize, 
			mac->currEdcaTable->qosList.dMaximumSize / (1024 * 1024), vinName, "DEQUEUE");
	}
