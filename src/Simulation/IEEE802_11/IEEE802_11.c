...
{
	init_linkpacketlog();
	fnNetSim_buffer_log_init();
	IEEE_802_11RadioMeasurements_Init();
...
{
	fn_NetSim_buffer_log_close();
	LinkPacketLog_close();
	IEEE_802_11RadioMeasurements_Finish();
...
{
	if (get_protocol_log_status("IEEE802_11_LOG"))
	return true;
	else
	return false;
}
...
{
	if (get_protocol_log_status("IEEE802_11_BACKOFF_LOG"))
	return true;
	else
	return false;
}
