...
{
	mac;
	return ceil(2 * (double)phy->plmeCharacteristics.aSIFSTime
							  ...
							  NetSim_PACKET* packet)
{
	...
	double aggregateSize = 0;

	while (packet != NULL)
	{
		if (packet->pstruMacData != NULL)
		{
			aggregateSize += (packet->pstruMacData->dPayload + 
				getMacOverhead(phy, packet->pstruMacData->dPayload));
		}
		packet = packet->pstruNextPacket;
	}
	double packetTime = fn_NetSim_IEEE802_11_CalculateTransmissionTime(
						aggregateSize,
																	   txId,
																	   txIf);
	...
	return ceil(get_preamble_time(phy) +
		rtsTime +
							  ...
							  double rtsduration)
{
	...
	double rtsTime = (getRTSSize() * 8.0) / phy->dControlFrameDataRate;
	return ceil(rtsduration -
		(phy->plmeCharacteristics.aSIFSTime +
			get_preamble_time(phy) +
			rtsTime));
}
