...
_declspec(dllexport)int fn_NetSim_Packet_AddPacketToBuffer(NetSim_BUFFER* pstruBuffer, NetSim_PACKET* pstruData)
{
	...
	if (nBufferFlag == Buffer_Underflow)
	{
		...
		if (pstruBuffer->nSchedulingType != SCHEDULING_NONE)
		{
			if (d)
			{
				if (pstruBuffer->dMaxBufferSize > pow(10, 6))
					fnNetSim_buffer_log_write("%lf,%d,%s,%d,Interface_%d (%s),%lf,%s,%s,%s", pstruEventDetails->dEventTime, d, DEVICE_NAME(d),
						DEVICE_INTERFACE_CONFIGID(d, in), DEVICE_INTERFACE_CONFIGID(d, in), DEVICE_INTERFACE(d, in)->szInterfaceName, pstruBuffer->dCurrentBufferSize, "inf", "N/A", "ENQUEUE");
				else
					fnNetSim_buffer_log_write("%lf,%d,%s,%d,Interface_%d (%s),%lf,%lf,%s,%s", pstruEventDetails->dEventTime, d, DEVICE_NAME(d),
						DEVICE_INTERFACE_CONFIGID(d, in), DEVICE_INTERFACE_CONFIGID(d, in), DEVICE_INTERFACE(d, in)->szInterfaceName, pstruBuffer->dCurrentBufferSize, pstruBuffer->dMaxBufferSize, "N/A", "ENQUEUE");
			}
		}
	...
	else if (nBufferFlag == Buffer_Overflow)
	{
		...
		if (pstruBuffer->nSchedulingType != SCHEDULING_NONE) 
		{
			if (d)
			{
				if (pstruBuffer->dMaxBufferSize > pow(10, 6))
					fnNetSim_buffer_log_write("%lf,%d,%s,%d,Interface_%d (%s),%lf,%s,%s,%s", pstruEventDetails->dEventTime, d, DEVICE_NAME(d),
						DEVICE_INTERFACE_CONFIGID(d, in), DEVICE_INTERFACE_CONFIGID(d, in), DEVICE_INTERFACE(d, in)->szInterfaceName, pstruBuffer->dCurrentBufferSize, "inf", "N/A", "DROP");
				else
					fnNetSim_buffer_log_write("%lf,%d,%s,%d,Interface_%d (%s),%lf,%lf,%s,%s", pstruEventDetails->dEventTime, d, DEVICE_NAME(d),
						DEVICE_INTERFACE_CONFIGID(d, in), DEVICE_INTERFACE_CONFIGID(d, in), DEVICE_INTERFACE(d, in)->szInterfaceName, pstruBuffer->dCurrentBufferSize, pstruBuffer->dMaxBufferSize, "N/A", "DROP");
			}
		}
...
_declspec(dllexport) NetSim_PACKET* fn_NetSim_GetPacketFromBuffer(NetSim_BUFFER* pstruBuffer, int nFlag)
{
	...
	if (pstruBuffer->nSchedulingType != SCHEDULING_NONE)
	{
		if (d)
		{
			if (pstruBuffer->dMaxBufferSize > pow(10, 6))
				fnNetSim_buffer_log_write("%lf,%d,%s,%d,Interface_%d (%s),%lf,%s,%s,%s", pstruEventDetails->dEventTime, d, DEVICE_NAME(d),
					DEVICE_INTERFACE_CONFIGID(d, in), DEVICE_INTERFACE_CONFIGID(d, in), DEVICE_INTERFACE(d, in)->szInterfaceName, pstruBuffer->dCurrentBufferSize, "inf", "N/A", "DEQUEUE");
			else
				fnNetSim_buffer_log_write("%lf,%d,%s,%d,Interface_%d (%s),%lf,%lf,%s,%s", pstruEventDetails->dEventTime, d, DEVICE_NAME(d),
					DEVICE_INTERFACE_CONFIGID(d, in), DEVICE_INTERFACE_CONFIGID(d, in), DEVICE_INTERFACE(d, in)->szInterfaceName, pstruBuffer->dCurrentBufferSize, pstruBuffer->dMaxBufferSize, "N/A", "DEQUEUE");
		}
	}
