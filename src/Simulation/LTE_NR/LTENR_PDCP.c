...
NETSIM_ID fn_NetSim_LTENR_PDCP_Splitting_Alogrithm(NETSIM_ID UEID, NETSIM_ID UEIF, ptrLTENR_PDCPENTITY entity) {
	...
	if (data->isDCEnable) {
		if (data->NSA_MODE == NSA_MODE_OPTION3A) {
				return rrc->SelectedCellID;
		}
		if (data->NSA_MODE == NSA_MODE_OPTION3 || data->NSA_MODE == NSA_MODE_OPTION7) {
			if (packet->nQOS == QOS_GBR)//GRB LTE
				return rrc->SelectedCellID;
			else if (packet->nQOS != QOS_GBR && rrc->SNID)
				return rrc->SNID;
			else
				return rrc->SelectedCellID;
		}
		if (data->NSA_MODE == NSA_MODE_OPTION4) {
			if (packet->nQOS == QOS_GBR && rrc->SNID)//GRB LTE
				return rrc->SNID;
			else if (packet->nQOS != QOS_GBR)
				return rrc->SelectedCellID;
			else
				return rrc->SelectedCellID;
		}
	...
	LTENR_RLCMODE rlcMode)
{
	...
	if (isUE(d, in))
		return LTENR_PDCP_FindEntity(pdcp, remote, rin, d, in, rlcMode);
	else if (isGNB(d, in))
		return LTENR_PDCP_FindEntity(pdcp, d, in, remote, rin, rlcMode);
	else
	{
		fnNetSimError("%d device is not an UE or gNB/eNB in function %s\n",
			d, __FUNCTION__);
		return NULL;
	}
}
	...
	LTENR_RLCMODE rlcMode)
{
	...
	ptrLTENR_PDCPENTITY entity = LTENR_PDCP_CreateNewEntity(gnbId, gnbIf, ueId, ueIf, rlcMode);
...
void LTENR_PDCP_HANDLE_TREORDERINGTIMER()
{
	...
	ptrLTENR_PDCPRXBUFFER_OPTIONAL opt = NULL;
	UINT32 next = 0;
...
static void LTENR_PDCP_TXBUFFER_ENQUEUE(ptrLTENR_PDCPENTITY entity,
	NetSim_PACKET* packet)
{
	if (LTENR_IS_UPLANE_MSG(packet) == false) return;
...
void fn_NetSim_LTENR_PDCP_TransmitPDCPSDU()
{
	...
	if (!msg)
	{
		fnNetSimError("LTENR-PDCP %s function is called without LTENR-MSG\n", __FUNCTION__);
		return;
	}

	ptrLTENR_PDCPVAR pdcp = LTENR_PDCP_GET(d, in);
	ptrLTENR_PDCPENTITY entity = LTENR_PDCPENTITY_GET(pdcp, d, in, packet->nReceiverId, msg->rlcMode);
	if (!entity)
	{
	...
	print_ltenr_log("En-queuing packet to transmission buffer.\n");
	...
	if (!isLTENRControlPacket(packet) && data->isDCEnable){
		if (!fn_NetSim_LTENR_NSA_IS_CORE_SPLIT_EXISTS(entity->ueId, entity->ueIf)) {
			if (isUE(d, in)) {
				...
				if (splitID != packet->nReceiverId && splitID) {
...
void fn_NetSim_LTENR_PDCP_ReceivePDCPSDU()
{
	...
	if (!fn_NetSim_LTENR_NSA_IS_CORE_SPLIT_EXISTS(d, in)) {
		if (isGNB(d, in) && !isLTENRControlPacket(packet)) {
			ptrLTENR_PDCPVAR pdcp = LTENR_PDCP_GET(d, in);
			pdcp->TotalPUSCHBytes += packet->pstruMacData->dPayload;
			ptrLTENR_PDCPVAR pdcpUE = LTENR_PDCP_GET(r, rin);
			pdcpUE->TotalPUSCHBytes += packet->pstruMacData->dPayload;
		...
		else if (isUE(d, in) && !isLTENRControlPacket(packet)) {
			ptrLTENR_PDCPVAR pdcp = LTENR_PDCP_GET(r, rin);
			pdcp->TotalPDSCHBytes += packet->pstruMacData->dPayload;
			ptrLTENR_PDCPVAR pdcpUE = LTENR_PDCP_GET(d, in);
			//pdcpUE->TotalPDSCHBytes += packet->pstruMacData->dPayload;
	...
	ptrLTENR_PDCPVAR pdcp = LTENR_PDCP_GET(d, in);
	ptrLTENR_PDCPENTITY entity = LTENR_PDCPENTITY_GET(pdcp, d, in, packet->nTransmitterId, msg->rlcMode);
	if (!entity)
	{
		fnNetSimError("No PDCP entity is found between device %d and %d\n"
			"Dropping packet in function %s\n",
...
{
	unsigned int i, j;
	double pdsch_throughput, pusch_throughput;
	PMETRICSNODE menu = init_metrics_node(MetricsNode_Menu, "LTENR_Cell_Metrics", NULL);
	PMETRICSNODE table = init_metrics_node(MetricsNode_Table, "LTENR_Cell_Metrics", NULL);

	add_table_heading(table, "gNB/eNB Name", true, 0);
	add_table_heading(table, "gNB/eNB Interface ID", true, 0);
	add_table_heading(table, "PDSCH Bytes Transmitted (bytes)", false, 0);
	add_table_heading(table, "PUSCH Bytes Transmitted (bytes)", false, 0);
	add_table_heading(table, "PDSCH Throughput (Mbps)", true, 0);
	add_table_heading(table, "PUSCH Throughput (Mbps)", true, 0);
	...
	for (i = 0; i < NETWORK->nDeviceCount; i++) {
		for (j = 0; j < NETWORK->ppstruDeviceList[i]->nNumOfInterface; j++) {
			...
			ptrLTENR_PDCPVAR pdcp = LTENR_PDCP_GET(d, in);
			if (!pdcp) return -1;
			pdsch_throughput = (pdcp->TotalPDSCHBytes * 8) / NETWORK->pstruSimulationParameter->dVal;
			pusch_throughput = (pdcp->TotalPUSCHBytes * 8) / NETWORK->pstruSimulationParameter->dVal;
			add_table_row_formatted(false, table, "%s,%d,%lf,%lf,%lf,%lf,",
				DEVICE_NAME(i + 1), j + 1, pdcp->TotalPDSCHBytes, pdcp->TotalPUSCHBytes,
				pdsch_throughput, pusch_throughput);
		}
	}
