...
void* LTENR_SDAP_DLHDR_COPY(ptrLTENR_SDAP_DLHDR hdr)
{
	ptrLTENR_SDAP_DLHDR dl = calloc(1, sizeof * dl);
	memcpy(dl, hdr, sizeof * dl);
	return dl;
}

void* LTENR_SDAP_ULHDR_COPY(ptrLTENR_SDAP_ULHDR hdr)
{
	ptrLTENR_SDAP_ULHDR ul = calloc(1, sizeof * ul);
	memcpy(ul, hdr, sizeof * ul);
	return ul;
}

void LTENR_SDAP_DLHDR_FREE(ptrLTENR_SDAP_DLHDR hdr)
{
	free(hdr);
}

void LTENR_SDAP_ULHDR_FREE(ptrLTENR_SDAP_ULHDR hdr) { free(hdr); }
#pragma endregion

#pragma region SDAP_INIT
void fn_NetSim_LTENR_SDAP_Init()
{
	sdapDLHDRId = LTENR_HDR_REGISTER("SDAP_DLHDR", NULL, LTENR_SDAP_DLHDR_COPY, LTENR_SDAP_DLHDR_FREE,NULL);
	sdapULHDRId = LTENR_HDR_REGISTER("SDAP_ULHDR", NULL, LTENR_SDAP_ULHDR_COPY, LTENR_SDAP_ULHDR_FREE,NULL);
...
void fn_NetSim_LTENR_SDAP_HandleMacOut()
{
	...
	if (!isFastProcessing && !isLTENRControlPacket(packet)) {
	...
	NetSim_PACKET * packet)
{
	...
	if (i != 0 && i != in) {
		...
		p->pstruNetworkData->dOverhead -= IPV4_HEADER_SIZE;
		p->pstruNetworkData->dPacketSize -= IPV4_HEADER_SIZE;
