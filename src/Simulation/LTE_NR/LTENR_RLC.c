	...
	bool isReceiving)
{
	...
	ptrLTENR_RLCENTITY entity = rlc ? rlc->rlcEntities : NULL;
	while (entity)
	{
	...
	bool isReceiving)
{
	...
	entity->retransmissionBuffer = LTENR_RLCBUFFER_ALLOC();
	entity->transmissionBuffer = LTENR_RLCBUFFER_ALLOC();
	entity->transmittedBuffer = LTENR_RLCBUFFER_ALLOC();
	...
	UINT size, LTENR_LOGICALCHANNEL logicalChannel)
{
	...
	while (buf)
	{
		...
		if (LTENR_IS_UPLANE_MSG(tmdpdu) &&
			tmdpdu->pstruMacData->dPacketSize > left)
		{
		...
		LTENR_RLCBuffer_MarkForRemoval(buf);
		buf = buf->next;
		left -= (UINT)tmdpdu->pstruMacData->dPacketSize;
