...
static ptrLTENR_CQITable LTENR_GetCQITableFromStr(char* table, UINT* tableId)
{
...
{
	phy->CSIReportConfig.cqiTable = LTENR_GetCQITableFromStr(table, &phy->CSIReportConfig.tableId);
}
...
{
	for (int i = LTENR_MAX_CQI; i > 0; i--)
	{
		if (efficiency >= table[i].efficiency)
			return table[i];
	}
	return table[0];
}
