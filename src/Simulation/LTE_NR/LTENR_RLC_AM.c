														...
														NetSim_PACKET* packet)
{
	...
	{
#ifdef LTENR_RLC_LOG
		print_ltenr_log("No reception buffer found for SN %d. Creating new buffer.\n", hdr->SN);
#endif
...
void LTENR_RLCAM_HandlePDUFromUpperLayer()
{
	...
	entity->AM.TX_Next++;
	entity->bufferStatus[channel] += (UINT)packet->pstruMacData->dPacketSize;
}

static ptrLTENR_RLCBUFFER_DATA LTENR_RLC_AM_FRAGMENT_PACKET(ptrLTENR_RLCBUFFER_DATA buffer, UINT size)
{
	if (size <= buffer->packet->pstruMacData->dOverhead + LTENR_RLCMSG_AMDHDR_SIZE_WITHSO) return NULL;
	size -= buffer->packet->pstruMacData->dOverhead + LTENR_RLCMSG_AMDHDR_SIZE_WITHSO;

	UINT s = (UINT)buffer->packet->pstruMacData->dPayload;
	ptrLTENR_RLCBUFFER_DATA ret;
	if (s > size)
	{
									 ...
									 NetSim_PACKET* amdpdu)
{
	...
	entity->AM.BYTE_WITHOUT_POLL += (UINT)amdpdu->pstruMacData->dPacketSize;
...
{
	ptrLTENR_RLCBUFFER_DATA buf = entity->retransmissionBuffer->head;
	...
	while (buf)
	{
		...
		if (amdpdu->pstruMacData->dPacketSize > left)
		{
			LTENR_RLCBUFFER_NEXT(buf);
			continue;
		}
		...
		if (left < (UINT)amdpdu->pstruMacData->dPacketSize)
			left = 0;
		else
			left -= (UINT)amdpdu->pstruMacData->dPacketSize;

		if (entity->bufferStatus[logicalChannel] < (UINT)amdpdu->pstruMacData->dPacketSize)
			entity->bufferStatus[logicalChannel] = 0;
		else
			entity->bufferStatus[logicalChannel] -= (UINT)amdpdu->pstruMacData->dPacketSize;

#ifdef LTENR_RLC_LOG
		print_ltenr_log("Sending packet to MAC sublayer. SI = %s, SN = %d, SO = %d\n",
		...
		ptrLTENR_RLCBUFFER_DATA data = buf->next;
		LTENR_RLCBUFFER_REMOVE_DATA(entity->retransmissionBuffer, buf);
		LTENR_RLCBUFFER_ADD_DATA(entity->transmittedBuffer, buf);
		buf = data;
	}
	return size - left;
}
...
static UINT LTENR_RLCAM_SendPDUFromTransmissionBuffer(ptrLTENR_RLCENTITY entity,
													  UINT size, LTENR_LOGICALCHANNEL logicalChannel)
{
	ptrLTENR_RLCBUFFER_DATA buf = entity->transmissionBuffer->head;
	UINT left = size;
	while (buf)
	{
		if (buf->channel != logicalChannel)
		...
		ptrLTENR_RLCBUFFER_DATA ret = LTENR_RLC_AM_FRAGMENT_PACKET(buf, left);
		if (ret == NULL) break;
		if (ret == buf)
		{
			buf = buf->next;
			LTENR_RLCBUFFER_REMOVE_DATA(entity->transmissionBuffer, ret);
		}
		...
		LTENR_RLCBUFFER_ADD_DATA(entity->transmittedBuffer, ret);
	}
	return size - left;
}
...
{
#ifdef LTENR_RLC_LOG
	print_ltenr_log("RLCAM-Poll retransmit timer triggered at time %lf ms.\n",
					pstruEventDetails->dEventTime/MILLISECOND);
#endif // LTENR_RLC_LOG
	...
	ptrLTENR_RLCBUFFER_DATA buffer = entity->transmittedBuffer->head;
	while (buffer)
	{
		if (buffer->SN <= entity->AM.POLL_SN)
		{
			...
			if (buffer->retxCount > entity->maxRetxThreshold)
			{
				...
				ptrLTENR_RLCBUFFER_DATA data = buffer->next;
				fn_NetSim_Packet_FreePacket(buffer->packet);
				LTENR_RLCBUFFER_REMOVE_DATA(entity->transmittedBuffer, buffer);
				free(buffer);
				buffer = data;
				continue;
			}
			...
			ptrLTENR_RLCBUFFER_DATA data = buffer->next;
			LTENR_RLCBUFFER_REMOVE_DATA(entity->transmittedBuffer, buffer);
			LTENR_RLCBUFFER_ADD_DATA(entity->retransmissionBuffer, buffer);
			entity->bufferStatus[buffer->channel] += (UINT)buffer->packet->pstruMacData->dPacketSize;
			buffer = data;
			continue;
		}
...
{
#ifdef LTENR_RLC_LOG
	print_ltenr_log("RLCAM-Status prohibit timer triggered at time %lf ms.\n",
					pstruEventDetails->dEventTime / MILLISECOND);
#endif // LTENR_RLC_LOG
...
static bool LTENR_RLCAM_MovePacketFromTransmittedBuffer(ptrLTENR_RLCENTITY entity, ptrLTENR_RLCBUFFER_DATA buffer)
{
	ptrLTENR_RLCMSG_AMDHDR hdr = LTENR_RLCMSG_AMHDR_GET(buffer->packet);
#ifdef LTENR_RLC_LOG
...
static void LTENR_RLCAM_UpdateBufferBasedOnNACK0(ptrLTENR_RLCENTITY entity, ptrLTENR_RLCMSG_STATUSPDU_NACK nack)
{
	...
	while (buffer->head)
	{
		ptrLTENR_RLCBUFFER_DATA data = LTENR_RLCBUFFER_GETHEAD(buffer);

		if (LTENR_RLCAM_MovePacketFromTransmittedBuffer(entity, data))
		{
...
static void LTENR_RLCAM_UpdateBufferBasedOnNACK1(ptrLTENR_RLCENTITY entity, ptrLTENR_RLCMSG_STATUSPDU_NACK nack)
{
	UINT sn = nack->NAck_SN;
	UINT16 so;
	ptrLTENR_RLCBUFFER_DATA buffer;
...
{
	ptrLTENR_RLCBUFFER_DATA buf = entity->transmittedBuffer->head;
	while (buf)
	{
		if (buf->SN < sn)
		{
			...
			LTENR_RLCBUFFER_REMOVE_DATA(entity->transmittedBuffer, buf);
			free(buf);
			buf = data;
		}
		else LTENR_RLCBUFFER_NEXT(buf);
	}
}
...
void LTENR_RLCAM_ReceiveStatusPDU()
{
	...
	if (entity->transmissionBuffer->head)
		entity->AM.TX_Next_Ack = entity->transmissionBuffer->head->SN;
	else
