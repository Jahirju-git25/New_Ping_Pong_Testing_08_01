...
int fn_NetSim_OLSR_UpdateRoutingTable()
{
	...
	while(topology)
	{
		ptrIP_ROUTINGTABLE table=iptable_check((ptrIP_ROUTINGTABLE*)&olsr->ipWrapper->table,topology->T_dest_addr,subnet);
		if(!table && IP_COMPARE(topology->T_dest_addr,olsr->mainAddress))
		{
			table=iptable_check((ptrIP_ROUTINGTABLE*)&olsr->ipWrapper->table,topology->T_last_addr,subnet);
