...
OLSR_2HOP_NEIGHBOR_SET* olsrFill2HopNeighbor(NODE_OLSR* olsr)
{
	...
	for(two_hop_neighbor = olsr->twoHopNeighborSet;two_hop_neighbor;two_hop_neighbor = (OLSR_2HOP_NEIGHBOR_SET*)LIST_NEXT(two_hop_neighbor))
	{
		...
		{
			OLSR_NEIGHBOR_SET* neighSet = olsrFindNeighborSet(olsr->neighborSet, two_hop_neighbor->N_2hop_addr);
			if (neighSet) continue; // Condition (ii)
...
void write_MPR_log(OLSR_MPR_SET* mprSet)
{
	//Code to write the log file
	NETSIM_ID nInterface;
	if (fpRMlog == NULL) return;
	fprintf(fpRMlog, "%.2lf,%s,%d,", pstruEventDetails->dEventTime / MILLISECOND, DEVICE_NAME(pstruEventDetails->nDeviceId), pstruEventDetails->nDeviceId);
	while (mprSet)
	{
		char ip[_NETSIM_IP_LEN];
		IP_TO_STR(mprSet->neighborAddress, ip);
		NETSIM_ID id = fn_NetSim_Stack_GetDeviceId_asIP(mprSet->neighborAddress, &nInterface);
		fprintf(fpRMlog, "%s; ", DEVICE_NAME(id));
		mprSet = (OLSR_MPR_SET*)LIST_NEXT(mprSet);
	}
	fprintf(fpRMlog, "\n");
