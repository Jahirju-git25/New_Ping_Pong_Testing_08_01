...
{
	int i =0;
	while(pstruSuperFrame->dSuperFrameStartTime + (double)i*aUnitBackoffPeriod*dUnitSymbolTime < dTime)
		i++;

	*dBoundaryTime = pstruSuperFrame->dSuperFrameStartTime + (double)i*aUnitBackoffPeriod*dUnitSymbolTime;
