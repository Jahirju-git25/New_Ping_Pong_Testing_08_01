...
#define PROPAGATION_CHANNEL_CHARACTERISTICS_DEFAULT		_strdup("NO_PATHLOSS")
...
*/
	...
	*/
	...
	typedef struct stru_tx_info
	{
		...
		double dChannelBandwidth; // In mHz
		double dTxPower_mw;
		double dTxPower_dbm;
		...
		double d0; // In m
		// Tx Sector Antenna Gain Parameters
		ANTENNA_TYPE Tx_AntennaType;
		double dTx_BoresightAngle;
		double dTx_FrontToBackRatio;
		double dTx_ElementGain;
		double dTx_Beamwidth;

		// Rx Sector Antenna Gain Parameters
		ANTENNA_TYPE Rx_AntennaType;
		double dRx_BoresightAngle;
		double dRx_FrontToBackRatio;
		double dRx_ElementGain;
		double dRx_Beamwidth;
	...
	//APIs to get radio measurment parameters
...
#define propagation_calculate_received_power(h,t,ti,r,ri,time) __pragma(message(__LOC__"propagation_calculate_received_power function is deprcated. Use _propagation_calculate_received_power")) _propagation_calculate_received_power(get_propagation_info(h,t,ti,r,ri),time)
	...
	_declspec(dllexport) double _propagation_calculate_lognormalshadow(double standardDeviation,
		double* Gset, bool* iSet);
	...
	_declspec(dllexport) double _propagation_calculate_logdistancepathloss(double pathLossExponent,
		double dTx_Rx_Distance, double dCentralFrequency, double d0);
	...
	_declspec(dllexport) void propagation_calculate_Two_Ray(PPROPAGATION_INFO pinfo,
		PRECV_POWER p);
	//Calculate Range Based path loss
	_declspec(dllexport) void propagation_calculate_Range_Based(PPROPAGATION_INFO pinfo,
		PRECV_POWER p);
	//Calculate Matrix Model
	_declspec(dllexport) void propagation_calculate_Matrix_Model(PPROPAGATION_INFO pinfo);

	void add_new_recv_power(PPROPAGATION_INFO info, double time, double loss);
...
#define propagation_calculate_fadingloss(h,t,ti,r,ri) __pragma(message(__LOC__"propagation_calculate_fadingloss function is deprcated. Use _propagation_calculate_fadingloss")) _propagation_calculate_fadingloss(get_propagation_info(h,t,ti,r,ri))
	...
	//Config reader
	_declspec(dllexport) int fn_NetSim_xmlConfigureWirelessMedium(void* xmlNetSimNode,
		struct stru_NetSim_Links* pstruLink);

	typedef void(*fnConfigureWirelessMedium)(void* xmlNetSimNode,
		struct stru_NetSim_Links* pstruLink);
	// Update Antenna Gain
	static void _propagation_update_antenna_gain(PPROPAGATION_INFO info);
