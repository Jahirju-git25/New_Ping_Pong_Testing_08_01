...
extern "C" {
	...
	enum enum_PacketType
	{
		...
		PacketType_BSM,
		PacketType_COAP,
		PacketType_Interactive_Gaming,
		PacketType_Control,
		PacketType_LAST, //Keep me at last
	};
	static const char* strPacketType[] = { "NULL","Voice","CBR","Custom","Database","Video","FTP",
	"HTTP","E-Mail","Peer-to-Peer","Erlang Call","Emulation","Sensing","Basic Safety Message","COAP","Interactive Gaming","Control_Packet","Unknown" };
	...
	enum enum_PacketStatus
	{
		...
		PacketStatus_LAST, //Keep me at last
	};
	static const char* strPacketStatus[] =
	{ "Successful","Errored","Collided","Buffer_Dropped","IP_TTL_Expired","Dropped","MAC_Lifetime_Expired","Unknown" };
	_declspec(dllexport) char* fn_NetSim_Config_GetPacketStatusAsString(PACKET_STATUS nStatus, char* buf);
...
//Used to create a new packet of any layer
	...
	_declspec(dllexport) int fn_NetSim_Packet_FreePacket_dbg(NetSim_PACKET** pstruPacket, int line, const char* file);
	...
	_declspec(dllexport) char* fn_NetSim_Config_GetPacketTypeAsString(PACKET_TYPE nPacketType, char* buffer);
