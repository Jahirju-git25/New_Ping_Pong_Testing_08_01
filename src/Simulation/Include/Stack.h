...
enum enum_AppProtocol
{
	...
	APP_PROTOCOL_INTERACTIVE_GAMING=PROTOCOL_APPLICATION,
};
...
typedef unsigned long long int (*fn_NetSim_Utilities_AddEvent)(struct stru_NetSim_EventDetails*,int,const char*); /* Add an event to event list.*/
_declspec(dllexport) int fnDeleteEvent(unsigned long long int nEventId);
_declspec(dllexport) void set_thread_safe_kernel(bool status);
...
typedef enum xmlDataType
{
	...
	_UINT8,
	_UINT16,
	_RATIO, //Double in form of a/b
	_SPECIALBOOL, // Enable = true, Disable = false
}XMLDATATYPE;
_declspec(dllexport) int GetXmlVal(void* var,const char* name,void* xmlNode,int flag, XMLDATATYPE type);
...
_declspec(dllexport) int fn_NetSim_Config_ConvertStrToEnum(void* xmlNode, const char* attributeName, const char** enumstr, const char* enumName, int length, bool isErrorMsg);
...
_declspec(dllexport) NETSIM_ID fn_NetSim_Stack_FindVirtualInterface(NETSIM_ID d, NETSIM_ID parentIf, char* name);
...
//Timer
_declspec(dllexport) UINT64 start_timer(NETSIM_ID d, NETSIM_ID in, UINT eventId, double time, void* data);
_declspec(dllexport) void cancel_timer(UINT64 eventId);
#ifdef  __cplusplus
}
#endif
