...
#define htonll(x) ((1==htonl(1)) ? (x) : ((uint64_t)htonl((x) & 0xFFFFFFFF) << 32) | htonl((x) >> 32))
	...
	//MatLab interface
	_declspec(dllexport) void netsim_matlab_interface_configure(char* appPath);
	_declspec(dllexport) void netsim_matlab_interface_close();
	_declspec(dllexport) void netsim_matlab_send_command(void* command, int len); //Don't use. Under development
	_declspec(dllexport) void netsim_matlab_send_ascii_command(char* format, ...);
	_declspec(dllexport) bool netsim_matlab_get_value(char* out, int outLen, char* name, char* type);
	_declspec(dllexport) struct ptrMXArray* netsim_matlab_get_array(char* name);
	_declspec(dllexport) void netsim_matlab_send_array(double* arr, size_t* shape, size_t dimn, char* name);
