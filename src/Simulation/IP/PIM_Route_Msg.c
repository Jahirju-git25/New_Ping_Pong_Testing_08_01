...
int pim_route_msg()
{
	bool isMsgArrivesFromBroadcastInterface = false;
	int ret = -1;
	...
	if (DEVICE_INTERFACE(d, in)->nInterfaceType != INTERFACE_WAN_ROUTER)
		isMsgArrivesFromBroadcastInterface = true;
	NetSim_PACKET* dummy = fn_NetSim_Packet_CreatePacket(NETWORK_LAYER);
	NETSIM_ID sid, sin;
	sid = fn_NetSim_Stack_GetDeviceId_asIP(src, &sin);
	dummy->pstruNetworkData->szDestIP = DEVICE_PUBLICIP(sid, sin);
	...
	if (!isMsgArrivesFromBroadcastInterface)
	{
	...
	for (i = 0; i < DEVICE(d)->nNumOfInterface; i++)
	{
		...
		if (isInPresentInRoute(d, i+1, route) && !isMsgArrivesFromBroadcastInterface)
			continue;
