...
static void add_new_dest(NetSim_PACKET* packet, NETSIM_IPAddress ip)
{
	...
	nw->szDestIP = ip;
	nw->Packet_NetworkProtocol = NULL;
	nw->Packet_RoutingProtocol = NULL;
}
...
static void remove_dest(NetSim_PACKET* packet)
{
	...
	if (nnw)
	{
		void* n = nnw->next;
		void* i = nnw->szDestIP;
		nw->Packet_NetworkProtocol = nnw->Packet_NetworkProtocol;
		nw->Packet_RoutingProtocol = nnw->Packet_RoutingProtocol;
