...
_declspec(dllexport) int fn_NetSim_IP_ICMP_Init()
{
	...
	for(i=0;i<NETWORK->nDeviceCount;i++)
	{
		if(NETWORK->ppstruDeviceList[i]->pstruNetworkLayer)
		{
			...
			if(devVar && devVar->nRouterAdvertisementFlag)
			{
																		  ...
																		  void(*callback)(NetSim_PACKET*))
{
	...
	packet->pstruNetworkData->dPacketSize = 8 + (double)size;
...
_declspec(dllexport) int fn_NetSim_IP_ICMP_GenerateDstUnreachableMsg()
{
	...
	// Added by PV on 11-11-20 to resolve ICMP related issue 3495
	packet->pstruNetworkData->szGatewayIP = IP_COPY(DEVICE_NWADDRESS(pstruEventDetails->nDeviceId,
