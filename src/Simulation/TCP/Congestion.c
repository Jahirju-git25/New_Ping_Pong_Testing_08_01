...
static inline UINT32 congestion_get_RCV_WND(PNETSIM_SOCKET s)
{
	PCONGESTIONVAR var = get_congestionvar(s);
	if (s->tcb->isWindowScaling)
	{
		UINT32 c = var->rwnd;
		return c << s->tcb->Rcv.Wind_Shift;
	}
	else
	{
		return var->rwnd;
...
UINT congestion_getDupAckCount(PNETSIM_SOCKET s)
{
	PCONGESTIONVAR var = get_congestionvar(s);
	return var->dupAckCount;
}
...
void congestion_setcallback(PNETSIM_SOCKET s)
{
	...
	case TCPVariant_OLDTAHOE:
		...
		s->tcb->get_dup_ack_count = congestion_getDupAckCount;
		break;
	...
	s->tcb->get_RCVWND = congestion_get_RCV_WND;
