	...
	case APPLICATION_IN_EVENT:
		{
				...
				pstruPacket->nControlDataType/100 != PROTOCOL_APPLICATION)
			{
				...
				print_application_log(pstruappinfo, pstruPacket);
	...
	case TIMER_EVENT:
		{
			...
			case event_APP_START:
				...
				case TRAFFIC_VIDEO:
				case TRAFFIC_HTTP:
				case TRAFFIC_INTERACTIVE_GAMING:
				case TRAFFIC_EMAIL:
				case TRAFFIC_CBR:
...
_declspec(dllexport) int fn_NetSim_Application_Finish()
{
	...
	for(loop=0;loop<nApplicationCount;loop++)
	{
		...
		if (applicationInfo[loop]->nAppType == TRAFFIC_VIDEO)
		{
			APP_VIDEO_INFO* info = applicationInfo[loop]->appData;
			free(info->dPacketSizeArgs);
		}
	...
	close_application_log();
	close_application_generation_log();
...
void update_latency()
{
	for (int i = 0; i < NETWORK->nApplicationCount; i++)
	{
		applicationInfo[i]->uplink.prevPacketLatency = -1;
		applicationInfo[i]->downlink.prevPacketLatency = -1;
		// Application Generation Logs
		applicationInfo[i]->ul.lastPacketSize = -1;
		applicationInfo[i]->ul.lastPacketTime = -1;
		applicationInfo[i]->ul.prevLastPacketTime = -1;
		applicationInfo[i]->dl.lastPacketSize = -1;
		applicationInfo[i]->dl.lastPacketTime = -1;
		applicationInfo[i]->dl.prevLastPacketTime = -1;
	}
}